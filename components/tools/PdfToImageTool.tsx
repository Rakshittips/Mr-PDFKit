...
export default function PdfToImageTool({ initialFile, onBack, onAddActivity, onSendToTool }: PdfToImageToolProps) {
  ...
  const handleConvert = async () => {
    ...
    setLoading(true)
    try {
      const res = await pdfToImages(file.buffer, { format, scale })
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-6">
        <ToolHeader
          title="PDF to"
          highlight="Images"
          description={`Rendered ${result.images.length} pages from ${file?.name}.`}
          onBack={() => {
            setResult(null)
          }}
        />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 bg-white dark:bg-zinc-900/60 border border-gray-100 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
          <div>
            <h3 className="text-base font-black text-gray-900 dark:text-white">
              {result.images.length} Pages Converted to {format.toUpperCase()}
            </h3>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex items-center gap-2 px-6 py-3.5 bg-lime-500 hover:bg-lime-600 text-white rounded-2xl font-black uppercase text-xs tracking-wider shadow-lg shadow-lime-500/20 active:scale-95 transition-all"
            >
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1.5 text-gray-400 hover:text-lime-500 rounded-lg hover:bg-lime-50 dark:hover:bg-lime-950/30 transition-colors"
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <ToolHeader
        title="PDF to"
        highlight="Image"
        description="Convert every page of your PDF into high-resolution PNG or JPEG images with crystal clear fidelity."
        onBack={onBack}
      />
      ...
      {!file ? (
        ...
        >
          <input
            ...
            className="hidden"
          />
          <div className="w-20 h-20 bg-lime-50 dark:bg-lime-900/20 text-lime-500 rounded-3xl flex items-center justify-center mx-auto mb-6">
            <UploadCloud size={36} />
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white dark:bg-zinc-900/60 border border-gray-100 dark:border-zinc-800 rounded-3xl p-6 flex items-center justify-between shadow-sm">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-2xl bg-lime-50 dark:bg-lime-900/20 text-lime-500 flex items-center justify-center">
                <FileText size={24} />
              </div>
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <div className="flex gap-2">
                  {[
                    { id: 'png', label: 'PNG (Lossless Quality)' },
                    { id: 'jpeg', label: 'JPEG (Compact Size)' },
                  ].map((f) => (
                    <button
                      key={f.id}
                      type="button"
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[
                    { val: 1.0, label: 'Standard (100%)' },
                    { val: 1.5, label: 'Medium (150%)' },
                    { val: 2.0, label: 'High (200% HD)' },
                  ].map((s) => (
                    <button
                      key={s.val}
                      type="button"
          ...
          <div className="flex justify-end">
            ...
            >
              {loading ? (
                <>
                  <Loader2 size={18} className="animate-spin" /> Rendering Pages...
                </>
              ) : (
                <>
                  <FileImage size={18} /> Convert {file.pageCount} Pages to {format.toUpperCase()}
                </>
              )}
            </button>
          </div>
